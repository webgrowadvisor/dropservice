                ...
                <div class="page-header-right ms-auto">
                    <div class="page-header-right-items">
                        ...
                        <div class="d-flex align-items-center gap-2 page-header-right-items-wrapper">
                            ...
                            <a href="{{ route('seller.order.create') }}" class="btn btn-primary">
                                        ...
                                        <tbody>
                                         @foreach($orders as $key => $order)
                                            <tr class="single-item">                                                
                                                ...
                                                <td>{!! variantImage($order->product->thumbnail,$order->variant?->webp, 60, 60) !!}</td>
                                                {{-- <td>{{ $order->user->name }}</td> --}}
                                                <td>{{ $order->product->name }}</td> 
                                                <td>{{ ucfirst($order->status) }}</td>
                                                <td>{{ $order->payment_method }}</td>
                                                <td>{{ $order->ip_address }}</td> 
                                                <td><a href="{{ route('seller.orders.show', $order->id) }}">View</a></td>                                           
                                                <td><a href="{{ route('seller.order.bill', $order->order_number) }}">Bill</a></td>                                           
                                                    ...
                                                    <div class="hstack gap-2 justify-content-end">
                                                        ...
                                                        <div class="dropdown">
                                                            ...
                                                            <ul class="dropdown-menu">
                                                                {{-- <li>
                                                                    <a class="dropdown-item" href="{{ route('seller.category.edit', $order->id) }}">
                                                                ...
                                                                <li>
                                                                    ...
                                                                    <form method="POST" action="{{ route('seller.orders.destroy', $order->id) }}">
                                                                        @csrf @method('DELETE')
