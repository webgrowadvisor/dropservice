            ...
            <div class="page-header">
                <div class="page-header-left d-flex align-items-center">
                    <div class="page-header-title">
                        <h5 class="m-b-10">Specifications Attributes</h5>
                    </div>
                </div>
                                        ...
                                        <tbody>
                                         @foreach($attributes as $key => $category)
                                            <tr class="single-item">                                                
                                                <td>
                                                {{ ($attributes->currentPage() - 1) * $attributes->perPage() + $key + 1 }}
                                                </td>
                                                <td>{{ $category->group->name }}</td>
                                                <td>{{ $category->name }}</td>
                                                <td>{{ $category->field_type }}</td>
                                                <td>{{ $category->default_value ?? '----' }}</td>
                                                <td>
                                                    <div class="hstack gap-2 justify-content-end">
                                                        ...
                                                        <div class="dropdown">
                                                            ...
                                                            <ul class="dropdown-menu">
                                                                <li>
                                                                    <a class="dropdown-item" href="{{ route('attributes.edit', $category->id) }}">
                                                                ...
                                                                <li>
                                                                    ...
                                                                    <form method="POST" action="{{ route('attributes.destroy', $category->id) }}">
                                                                        @csrf @method('DELETE')
